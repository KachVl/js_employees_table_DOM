...
function sortTable(columnIndex) {
    ...
    while(switching){
        ...
        if (sortingIndex !== columnIndex) {
            for(i = 0; i < rows.length - 1; i++){
                ...
                if (columnIndex === 4) {
                    if (+row1.innerHTML.slice(1).replaceAll(",", "") > +row2.innerHTML.slice(1).replaceAll(",", "")) {
                        shouldSwitch = true;
                        break;
                    }
    ...
    if (sortingIndex === columnIndex) sortingIndex = null;
    else sortingIndex = columnIndex;
}
...
newEmployeeForm.append(nameLabel, positionLabel, officeLabel, ageLabel, salaryLabel);
...
officeSelect.append(officeOption1, officeOption2, officeOption3, officeOption4, officeOption5, officeOption6);
...
const setEditableCells = ()=>{
    const editableCells = tableContent.querySelectorAll("td");
...
function addNewEmployee() {
    const warning = [];
    if (nameInput.value.length < 4) warning.push("Name should be at least 4 symbols length.");
    ...
    if (warning.length > 0) return showNotify(warning.join("\n"), "error");
